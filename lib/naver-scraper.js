...
async function getBrowser() {
  ...
  if (BROWSERLESS_TOKEN) {
    const { chromium } = await import("playwright-core");
    const browserlessUrl = `wss://${BROWSERLESS_REGION}.browserless.io?token=${BROWSERLESS_TOKEN}`;
    return chromium.connectOverCDP(browserlessUrl);
  }

  if (process.env.VERCEL) {
    throw new Error(
      "Vercel serverless runtime cannot launch the bundled Chromium in this deployment. Set BROWSERLESS_TOKEN, PLAYWRIGHT_WS_ENDPOINT, or CHROME_CDP_URL to use a remote browser."
    );
  }
